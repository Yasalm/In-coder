...
def has_encoder(encoder_name):
    session = next(get_session())
    if session.exec(select(Encoder).where(Encoder.name == encoder_name)).first(): return True
    return False


def is_collection(collection_name):
    session = next(get_session())
    if session.exec(select(Collection).where(Collection.name == collection_name)).first(): return True
    return False


def get_encoders(collection_name):
    session = next(get_session())
    result = session.exec(select(Collection).where(Collection.name == collection_name)).first()
